...
const programTypes = ['Global Entry', 'TSA', 'SENTRI', 'NEXUS'];
...
const formSchema = z.object({
  ...
  knownTravelerNumber: commonFieldSchema().optional(),
  knownTravelerCountryCode: commonFieldSchema().optional(),
  appointmentDatePreference: commonFieldSchema(),
});

const GlobalEntryTSAForm = ({
  isView,
  platformServiceId,
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      knownTravelerNumber: defaultData?.serviceFields?.knownTravelerNumber || '',
      knownTravelerCountryCode: defaultData?.serviceFields?.knownTravelerCountryCode || '',
      appointmentDatePreference: defaultData?.serviceFields?.appointmentDatePreference || '',
    },
  });
                ...
                <FormControl>
                  <PhoneInput2
                    ...
                    onChange={(val, df) => {
                      ...
                      form.setValue('knownTravelerCountryCode', `+${df.dialCode || ''}`);
                    }}
                    disabled={isView}
                  />
                </FormControl>
...
export default GlobalEntryTSAForm;
